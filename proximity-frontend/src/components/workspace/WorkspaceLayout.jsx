...
export default function WorkspaceLayout({ sidebar, header, metrics, tabs, children, footer, mobileOpen = false, onMobileClose }) {
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: 'calc(100vh - 64px)', bgcolor: workspaceTokens.shell.background }}>
      {desktop ? <Box sx={{ width: SIDEBAR_WIDTH, flexShrink: 0, borderRight: `1px solid ${workspaceTokens.shell.border}`, bgcolor: workspaceTokens.shell.surface }}>{sidebar}</Box> : <Drawer open={mobileOpen} onClose={onMobileClose} ModalProps={{ keepMounted: true }} PaperProps={{ sx: { width: SIDEBAR_WIDTH } }}>{sidebar}</Drawer>}
